...
class MutliHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        query = query.view(
            query.shape[0], query.shape[1], self.nheads, self.d_k).transpose(1, 2)
        key = key.view(key.shape[0], key.shape[1],
                       self.nheads, self.d_k).transpose(1, 2)
        value = value.view(
            value.shape[0], value.shape[1], self.nheads, self.d_k).transpose(1, 2)

        x, self.attention_scores = MutliHeadAttentionBlock.attention(
            query, key, value, mask, self.dropout)

        # [batch, nheads, seq_len, d_k] -> [batch, seq_len, nheads, d_k] -> [batch, seq_len, d_model]
        x = x.transpose(1, 2).contiguous().view(
            x.shape[0], -1, self.nheads * self.d_k)
...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        # first normalize, then use the sublayer
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_out, src_mask, tgt_mask):
        ...
        x = self.residual_connections[1](
            x, lambda x: self.cross_attention_block(x, encoder_out, encoder_out, None))
...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder: Encoder, decoder: Decoder, src_norm: LayerNormalization, lin_proj: LinearProjection) -> None:
        super().__init__()
...
def build_transformer(d_model: int, Nx: int = 2, nheads: int = 2, dropout: float = 0.15, d_ff: int = 128) -> Transformer:
    ...
    encoder_blocks = []
    for _ in range(Nx):
        encoder_self_attention_block = MutliHeadAttentionBlock(
            d_model, nheads, dropout)
        ff_block = FeedForwardBlock(d_model, d_ff, dropout)
        encoder_block = EncoderBlock(
            encoder_self_attention_block, ff_block, dropout)
        encoder_blocks.append(encoder_block)

    # create the decoder blocks
    decoder_blocks = []
    for _ in range(Nx):
        decoder_self_attention_block = MutliHeadAttentionBlock(
            d_model, nheads, dropout)
        decoder_cross_attention_block = MutliHeadAttentionBlock(
            d_model, nheads, dropout)
        ff_block = FeedForwardBlock(d_model, d_ff, dropout)
    ...
    transformer_model = Transformer(
        encoder, decoder, src_norm, projection_layer)
