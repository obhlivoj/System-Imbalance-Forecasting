...
def run_validation(model, config, validation_dataloader, scaler, device, print_msg, epoch):
    model.eval()
...
def get_ds(cfg, train_bs=None):
    ...
    train_data_raw, train_data_tensor_enc, train_data_tensor_dec, train_label_tensor = prepare_time_series_data(
        data_train, cfg["exo_vars"], cfg["exo_vars"], cfg["target"], cfg['src_seq_len'], cfg['tgt_seq_len'])
    val_data_raw, _, _, _ = prepare_time_series_data(
        data_val, cfg["exo_vars"], cfg["exo_vars"], cfg["target"], cfg['src_seq_len'], cfg['tgt_seq_len'])
...
def get_model(config):
    model = build_transformer(d_model=len(
        config['exo_vars'])+1, d_ff=config["d_ff"])
    return model
...
def train_model(config):
    ...
    scheduler = lr_scheduler.LinearLR(
        optimizer, start_factor=1.0, end_factor=0.01, total_iters=8)
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        val_loss, _, _, _ = run_validation(model, config, val_dataloader, scaler,
                                           device, lambda msg: batch_iterator.write(msg), epoch)
