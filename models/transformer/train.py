...
def get_ds(config, train_bs=None, return_raw=False):
    ...
    train_data_raw, train_data_tensor, train_label_tensor = prepare_time_series_data(
        data_train, config["exo_vars"], config["target"], config['src_seq_len'], config['tgt_seq_len'])
    val_data_raw, _, _ = prepare_time_series_data(
        data_val, config["exo_vars"], config["target"], config['src_seq_len'], config['tgt_seq_len'])
    test_data_raw, _, _ = prepare_time_series_data(
        data_test, config["exo_vars"], config["target"], config['src_seq_len'], config['tgt_seq_len'])
    ...
    data_scaled, label_scaler = scale_data_seq(
        train_data_tensor, train_label_tensor, data_to_scale)
    ...
    val_dataloader_onebatch = DataLoader(val_ds, batch_size=1, shuffle=False)
